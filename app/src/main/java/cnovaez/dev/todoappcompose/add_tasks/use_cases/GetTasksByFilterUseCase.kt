 ...
 **/
class GetTasksByFilterUseCase @Inject constructor(private val tasksRepository: TasksRepository) {
    suspend operator fun invoke(filter: String, date: String): List<TaskEntity> = tasksRepository.getTaskByFilter(filter, date)
}